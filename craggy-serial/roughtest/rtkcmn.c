...
const static double glo0 []={1970,1, 1,0,0,0}; /* beidou time reference */
...
* return : gtime_t struct
*-----------------------------------------------------------------------------*/
extern gtime_t glo2time(int N4, int Nt, double sec)
{
    double glo []={1996 + (N4-1)*4 ,1, 1,0,0,0};
    gtime_t t=epoch2time(glo);

    if (sec<-1E9||1E9<sec) sec=0.0;
    t.time+=86400*(Nt-1) + (int)sec - 3600*3;
    t.sec=sec-(int)sec;
    return t;
}
