...
#define ID_NAVSTAT 0x0103 /* ubx message id: nav solution info */
#define ID_NAVPVT 0x0107 /* ubx message id: nav pvt*/

#define ID_NAVTIME 0x0120  /* ubx message id: nav time gps */
#define ID_NAVDOPS 0x0104  /* ubx message id: gps dops */
#define ID_NAVCLOCK 0x0122 /* ubx message id: clock solution */

#define ID_TIMEGLO 0x0123 /* ubx message id: nav time gps */
#define ID_TIMEGST 0x0125 /* ubx message id: nav time galileo */
#define ID_TIMEBDT 0x0124 /* ubx message id: nav time beidou */
...
static int decode_rxmraw(raw_t *raw)
{
    ...
    fprintf(stdout, "decode_rxmraw: len=%d\n", raw->len);
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmraw length error: len=%d nsat=%d\n", raw->len, nsat);
#endif
        return -1;
    }
    tow = U4(p);
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmraw week=0 error: len=%d nsat=%d\n", raw->len, nsat);
#endif
        return 0;
    }
    /* time tag adjustment */
    ...
    for (i = 0, p += 8; i < nsat && i < MAXOBS; i++, p += 24)
    {
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx rxmraw sat number error: prn=%d\n", prn);
#endif
            continue;
        }
...
static int decode_rxmrawx(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_rxmrawx: len=%d\n", raw->len);
#endif
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmrawx length error: len=%d nsat=%d\n", raw->len, nsat);
#endif
        return -1;
    }
    tow = R8(p);
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmrawx week=0 error: len=%d nsat=%d\n", raw->len, nsat);
#endif
        return 0;
    }
    if (raw->outtype)
    ...
    for (i = 0, p += 16; i < nsat && i < MAXOBS; i++, p += 32)
    {
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx rxmrawx: system error\n");
#endif
            continue;
        }
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx rxmrawx sat number error: sys=%2d prn=%2d\n", sys, prn);
#endif
            continue;
        }
...
#else
            if (halfc != raw->halfc[sat - 1][0])
                raw->obs.data[n].LLI[0] |= LLI_SLIP;
#endif
...
#ifdef dbg
    fprintf(stdout, "save_subfrm: sat=%2d id=%d\n", sat, id);
#endif
...
#ifdef dbg
    fprintf(stdout, "decode_ephem: sat=%2d\n", sat);
#endif
...
#ifdef dbg
    fprintf(stdout, "decode_alm1 : sat=%2d\n", sat);
#endif
...
#ifdef dbg
    fprintf(stdout, "decode_alm2 : sat=%2d\n", sat);
#endif
...
static int decode_rxmsfrb(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_rxmsfrb: len=%d\n", raw->len);
#endif
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmsfrb length error: len=%d\n", raw->len);
#endif
        return -1;
    }
    prn = U1(p + 1);
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmsfrb satellite number error: prn=%d\n", prn);
#endif
        return -1;
    }
    sys = satsys(sat, &prn);
...
static int decode_navstat(raw_t *raw)
{
    unsigned char *p = raw->buff + 6;

#ifdef dbg
    fprintf(stdout, "decode_navstat: len=%d\n", raw->len);
#endif

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-STAT   (%4d):", raw->len);
    }
    raw->rxstat.iTow = U4(p);
    raw->rxstat.gpsFix = U1(p + 4);
    raw->rxstat.flags1 = U1(p + 5);
    raw->rxstat.fixStat = U1(p + 6);
    raw->rxstat.flags2 = U1(p + 7);
    raw->rxstat.ttff = U4(p + 8);
    raw->rxstat.msss = U4(p + 12);
    return 0;
}

/* decode ubx-nav-pvt: PVT solution -----------------------------------*/
static int decode_navpvt(raw_t *raw)
{
    unsigned char *p = raw->buff + 6;

#ifdef dbg
    fprintf(stdout, "decode_navpvt: len=%d\n", raw->len);
#endif
    ...
    raw->pvt.iTow = U4(p);
    raw->pvt.year = U2(p+4);
    raw->pvt.month = U1(p+6);
    raw->pvt.day = U1(p+7);
    raw->pvt.hour = U1(p+8);
    raw->pvt.min = U1(p+9);
    raw->pvt.sec = U1(p+10);
    raw->pvt.valid = U1(p+11);
    raw->pvt.tAcc = U4(p+12);
    raw->pvt.nano = I4(p+16);
    raw->pvt.fixType = U1(p+20);
    raw->pvt.flags = U1(p+21);
    raw->pvt.flags2 = U1(p+22);
    raw->pvt.numSv = U1(p+23);
    raw->pvt.lon = I4(p+24);
    raw->pvt.lat = I4(p+28);
    raw->pvt.height = I4(p);
    raw->pvt.hMSL = I4(p+32);
    raw->pvt.hACC = U4(p+36);
    raw->pvt.vACC = U4(p+40);
    raw->pvt.velN = I4(p+44);
    raw->pvt.velE = I4(p+48);
    raw->pvt.velD = I4(p+52);
...
#ifdef dbg
    fprintf(stdout, "decode_navsol: len=%d\n", raw->len);
#endif
...
{
    int itow, ftow, week, leaps;
    unsigned char *p = raw->buff + 6;

#ifdef dbg
    fprintf(stdout, "decode_navtime_gps: len=%d\n", raw->len);
#endif
    ...
    {
        fprintf(stdout, "Time not valid - Flag %d\n", U1(p + 11));
    }
#endif
    return 0;
}

static int decode_navtime_gal(raw_t *raw)
{
    int galWno, galTow, fGalTow, leaps;
    unsigned char *p = raw->buff + 6;

#ifdef dbg
    fprintf(stdout, "decode_navtime_gal: len=%d\n", raw->len);
#endif
    ...
    galWno = I2(p + 12);
    leaps = U1(p + 14);
    if ((U1(p + 15) & 0x07) == 0x07)
    {
        raw->time_gal = gst2time(galWno, galTow + fGalTow * 1E-9);
    }
#ifdef dbg
    else
    {
...
static int decode_navtime_bdt(raw_t *raw)
{
    int bdtWno, ftow, bdtTow, leaps;
    unsigned char *p = raw->buff + 6;

#ifdef dbg
    fprintf(stdout, "decode_navtime_bdt: len=%d\n", raw->len);
#endif
    ...
    if ((U1(p + 15) & 0x07) == 0x07)
    {
        raw->time_bdt = bdt2time(bdtWno, bdtTow + ftow * 1E-9);
    }
#ifdef dbg
    else
    {
...
static int decode_navtime_glo(raw_t *raw)
{
    int tod, ftod, N4, Nt;
    unsigned char *p = raw->buff + 6;
    ...
    Nt = U2(p + 12);
    N4 = U1(p + 14);
    if ((U1(p + 15) & 0x03) == 0x03)
    {
        raw->time_glo = glo2time(N4, Nt, tod + ftod * 1e-9);
    }
    return 0;
}
...
static int decode_trkmeas(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_trkmeas: len=%d\n", raw->len);
#endif
    ...
    {
#ifdef dbg
        fprintf(stdout, "decode_trkmeas: length error len=%d nch=%2d\n", raw->len, nch);
#endif
        return -1;
    }
    ...
    for (i = 0, p = raw->buff + 110; i < nch; i++, p += 56)
    {
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx trkmeas: system error\n");
#endif
            continue;
        }
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx trkmeas sat number error: sys=%2d prn=%2d\n", sys, prn);
#endif
            continue;
        }
        /* transmission time */
...
#if 0 /* for debug */
#ifdef dbg
        fprintf(stdout,"[%2d] qi=%d sys=%d prn=%3d frq=%2d flag=%02X ?=%02X %02X "
#endif
...
static int decode_trkd5(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_trkd5: len=%d\n", raw->len);
#endif
    ...
    for (i = 0, p = raw->buff + off; p - raw->buff < raw->len - 2; i++, p += len)
    {
        ...
        if (type == 6)
        {
            ...
            {
#ifdef dbg
                fprintf(stdout, "ubx trkd5: system error\n");
#endif
                continue;
            }
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx trkd5 sat number error: sys=%2d prn=%2d\n", sys, prn);
#endif
            continue;
        }
        /* transmission time */
...
#if 0 /* for debug */
#ifdef dbg
        fprintf(stdout,"[%2d] qi=%d sys=%d prn=%3d frq=%2d flag=%02X ts=%1.3f "
#endif
...
static int decode_nav(raw_t *raw, int sat, int off)
{
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx length error: sat=%d len=%d\n", sat, raw->len);
#endif
        return -1;
    }
    for (i = 0; i < 10; i++, p += 4)
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx subfrm id error: sat=%2d\n", sat);
#endif
        return -1;
    }
    for (i = 0; i < 10; i++)
...
static int decode_enav(raw_t *raw, int sat, int off)
{
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx gal page even/odd error: sat=%2d\n", sat);
#endif
        return -1;
    }
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx gal page crc error: sat=%2d\n", sat);
#endif
        return -1;
    }
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx gal svid error: sat=%2d %2d\n", sat, eph.sat);
#endif
        return -1;
    }
    if (!strstr(raw->opt, "-EPHALL"))
...
static int decode_cnav(raw_t *raw, int sat, int off)
{
    ...
    { /* GEO */
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx rawsfrbx page number error: sat=%2d\n", sat);
#endif
            return -1;
        }
        for (i = 0; i < 10; i++)
...
static int decode_gnav(raw_t *raw, int sat, int off, int frq)
{
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx gnav length error: len=%d\n", raw->len);
#endif
        return -1;
    }
    for (i = k = 0; i < 4; i++, p += 4)
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx glo string hamming error: sat=%2d\n", sat);
#endif
        return -1;
    }
    m = getbitu(buff, 1, 4);
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx glo string no error: sat=%2d\n", sat);
#endif
        return -1;
    }
...
static int decode_snav(raw_t *raw, int sat, int off)
{
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rawsfrbx snav length error: len=%d\n", raw->len);
#endif
        return -1;
    }
...
static int decode_rxmsfrbx(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_rxmsfrbx: len=%d\n", raw->len);
#endif
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmsfrbx sys id error: sys=%d\n", U1(p));
#endif
        return -1;
    }
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx rxmsfrbx sat number error: sys=%d prn=%d\n", sys, prn);
#endif
        return -1;
    }
    switch (sys)
...
static int decode_trksfrbx(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_trksfrbx: len=%d\n", raw->len);
#endif
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx trksfrbx sys id error: sys=%d\n", U1(p + 1));
#endif
        return -1;
    }
    ...
    {
#ifdef dbg
        fprintf(stdout, "ubx trksfrbx sat number error: sys=%d prn=%d\n", sys, prn);
#endif
        return -1;
    }
    switch (sys)
...
static int decode_navdops(raw_t *raw)
{
    unsigned char *p = raw->buff + 6;
#ifdef dbg
    fprintf(stdout, "decode_navdops: len=%d\n", raw->len);
#endif
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-DOPS  (%4d):", raw->len);
    }
    raw->dops.pDOP = U2(p + 6);
    raw->dops.tDOP = U2(p + 8);
    raw->dops.vDOP = U2(p + 10);
    raw->dops.hDOP = U2(p + 12);
    raw->dops.nDOP = U2(p + 14);
    raw->dops.eDOP = U2(p + 16);

    return 0;
}

static int decode_navclock(raw_t *raw)
{
    unsigned char *p = raw->buff + 6;
#ifdef dbg
    fprintf(stdout, "decode_nav: len=%d\n", raw->len);
#endif
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-DOP (%4d):", raw->len);
    }
    raw->navclk.clkB = I4(p + 4);
    raw->navclk.clkD = I4(p + 8);
    raw->navclk.tAcc = U4(p + 12);
    raw->navclk.fAcc = U4(p + 16);
...
static int decode_ubx(raw_t *raw)
{
...
#ifdef dbg
    fprintf(stdout, "decode_ubx: type=%04x len=%d\n", type, raw->len);
#endif

    /* checksum */
    if (!checksum(raw->buff, raw->len))
    {
        fprintf(stdout, "ubx checksum error: type=%04x len=%d\n", type, raw->len);
        return -1;
    }
    switch (type)
    {
    ...
    case ID_TIMEGST:
        return decode_navtime_gal(raw);
    case ID_TIMEBDT:
        return decode_navtime_bdt(raw);
    case ID_TIMEGLO:
        return decode_navtime_glo(raw);
    case ID_NAVDOPS:
        return decode_navdops(raw);
    case ID_NAVPVT:
...
extern int input_ubx(raw_t *raw, unsigned char data)
{
    ...
    if (raw->nbyte == 6)
    {
        ...
        {
#ifdef dbg
            fprintf(stdout, "ubx length error: len=%d\n", raw->len);
#endif
            raw->nbyte = 0;
            return -1;
        }
