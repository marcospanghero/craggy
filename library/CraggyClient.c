...
bool craggy_createRequest(craggy_rough_time_nonce_t nonce, craggy_rough_time_request_t requestBuf) {
    ...
    size_t paddingLen = CRAGGY_ROUGH_TIME_MIN_REQUEST_SIZE - (CRAGGY_ROUGHTIME_MESSAGE_HEADER_SIZE + craggy_messageHeaderLen(3) + CRAGGY_ROUGH_TIME_NONCE_LENGTH + sizeof(uint32_t));
    uint8_t padding[paddingLen];
    craggy_memset(padding, 0, paddingLen);

    CraggyRoughtimeMessageBuilder *builder = NULL;

    if (craggy_createMessageBuilder(3, requestBuf+CRAGGY_ROUGHTIME_MESSAGE_HEADER_SIZE, CRAGGY_ROUGH_TIME_MIN_REQUEST_SIZE-12, &builder)) {
        if (craggy_addTagData(builder, CRAGGY_TAG_PAD, padding, paddingLen)) {
            ...
            if (craggy_addTagData(builder, CRAGGY_TAG_VER, (uint8_t *) &version, sizeof(uint32_t))) {
                if (craggy_addTagData(builder, CRAGGY_TAG_NONCE, nonce, CRAGGY_ROUGH_TIME_NONCE_LENGTH)) {
                    ...
                    assert(requestBufLen == (CRAGGY_ROUGH_TIME_MIN_REQUEST_SIZE-CRAGGY_ROUGHTIME_MESSAGE_HEADER_SIZE));
...
#define HASH_NONCE(hash, scratch,leaf) scratch[0] = '\x00'; craggy_memcpy((scratch)+1,(leaf), CRAGGY_ROUGH_TIME_NONCE_LENGTH); if (!craggy_calculateSHA512((scratch), CRAGGY_ROUGH_TIME_NONCE_LENGTH+1, (hash))) { ERROR_OCCURRED(CraggyResultInternalError) };
 ...
 */
    ...
    /** 1. Verify the signature in the certificate of the delegation message. */
    if (!craggy_getFixedLenTag(certMessage, CRAGGY_TAG_SIG, CRAGGY_ROUGH_TIME_SIGNATURE_LENGTH, &signature)) {
    ...
    if (!craggy_getFixedLenTag(delegationMessage, CRAGGY_TAG_PUBK, CRAGGY_ROUGH_TIME_PUBLIC_KEY_LENGTH,
                               &delegationPublicKey)) {
        ERROR_OCCURRED(CraggyResultParseErrorTagSizeMismatch);
    }
    if (!craggy_getFixedLenTag(message, CRAGGY_TAG_SIG, CRAGGY_ROUGH_TIME_SIGNATURE_LENGTH, &signature)) {
        ERROR_OCCURRED(CraggyResultParseErrorTagSizeMismatch);
    }
    ...
    if (!craggy_getFixedLenTag(srepMessage, CRAGGY_TAG_ROOT, CRAGGY_ROUGH_TIME_HASH_LENGTH, &rootHash)) {
        ERROR_OCCURRED(CraggyResultParseErrorTagSizeMismatch);
    }
    ...
    assert((pathSize & (uint32_t) CRAGGY_ROUGH_TIME_HASH_LENGTH) == 0);
    ...
    while (pathSize > 0) {
        ...
        index >>= (uint32_t) 1;
        pathSize -= CRAGGY_ROUGH_TIME_HASH_LENGTH;
        path += CRAGGY_ROUGH_TIME_HASH_LENGTH;
    }
    assert(pathSize == 0);
    ...
    if (!craggy_getFixedLenTag(srepMessage, CRAGGY_TAG_MIDP, sizeof(craggy_rough_time_t), &nestedData)) {
        ERROR_OCCURRED(CraggyResultParseErrorTagSizeMismatch);
    }
    craggy_memcpy(&midPoint, nestedData, sizeof(craggy_rough_time_t));

    craggy_rough_time_t minTime;
    if (!craggy_getFixedLenTag(delegationMessage, CRAGGY_TAG_MINT, sizeof(craggy_rough_time_t), &nestedData)) {
        ERROR_OCCURRED(CraggyResultParseErrorTagSizeMismatch);
    }
    craggy_memcpy(&minTime, nestedData, sizeof(craggy_rough_time_t));

    craggy_rough_time_t maxTime;
    if (!craggy_getFixedLenTag(delegationMessage, CRAGGY_TAG_MAXT, sizeof(craggy_rough_time_t), &nestedData)) {
        ERROR_OCCURRED(CraggyResultParseErrorTagSizeMismatch);
    }
    ...
    craggy_memcpy(&roughtimeResult->time, &midPoint, sizeof(craggy_rough_time_t));
