...
void gpsd_zero_satellites(struct gps_data_t *out)
{
    ...
    {
        out->skyview[sat].azimuth = 0;
        out->skyview[sat].elevation = 0;
        out->skyview[sat].ss = 0;
        out->skyview[sat].freqid = -1;
    }
}

void gpsd_zero_raw(struct gps_data_t *out)
{
    int raw;

    (void)memset(out->raw.meas, '\0', sizeof(out->raw.meas));
    out->raw.avb_meas = 0;
    // zero is good inbound data for ss, elevation, and azimuth.
    // we need to set them to invalid values
    for (raw = 0; raw < MAXCHANNELS; raw++)
    {
        out->raw.meas[raw].c2c = 0;
        out->raw.meas[raw].codephase = 0;
        out->raw.meas[raw].intCodePhase = 0;
        out->raw.meas[raw].fracChips = 0;
        out->raw.meas[raw].wholeChips = 0;
        out->raw.meas[raw].pseudorange = 0;
        out->raw.meas[raw].svid = 0;
        out->raw.meas[raw].doppler = 0;
        out->raw.meas[raw].dopplerMS = 0;
    }
}
...
void gps_clear_fix(struct gps_fix_t *fixp)
{
    ...
    fixp->clock_bias = 0;
    fixp->clock_drift = 0;
    fixp->fAcc_estimate = 0;
    fixp->tAcc_estimate = 0;
    fixp->gps_time_ftow = 0;
    fixp->gps_time_itow = 0;
    fixp->gps_time_weekn = 0;
                   ...
                   struct gps_fix_t *from)
{
    ...
    {
        to->clock_bias = from->clock_bias;
        to->clock_drift = from->clock_drift;
        to->tAcc_estimate = from->tAcc_estimate;
        to->fAcc_estimate = from->fAcc_estimate;
    }

    if (0 != (transfer & TIME_SET))
    {
        to->time = from->time;
        to->gps_time_ftow = from->gps_time_ftow;
        to->gps_time_itow = from->gps_time_itow;
        to->gps_time_weekn = from->gps_time_weekn;
    }
                                   ...
                                   double *ib, double *fb)
{
    ...
    d_S = B * s_S * (c_2SM + B / 4 * (c_S * (-1 + 2 * c_2SM * c_2SM) - B / 6 * c_2SM * (-3 + 4 * s_S * s_S) * (-3 + 4 * c_2SM * c_2SM)));
