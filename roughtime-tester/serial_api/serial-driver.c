...
void
timespec_add(struct timespec *r, const struct timespec *a,
...
int serial_port_open_port(serial_port_t *serial_port_config){
    ...
    serial_port_config->port_descriptor = open(serial_port_config->port_name, O_RDWR | O_NOCTTY);

    if (serial_port_config->port_descriptor < 0) {
        fprintf(stderr, "Could not open port %s \n",serial_port_config->port_name);
        return 1;
    } else {
        cfsetispeed(&serial_port_config->port_descriptor, B9600);
...
int serial_port_set_raw_8bit(serial_port_t *serial_port_config) {
    ...
    if (serial_port_config->is_open == 1) {
        *(&(serial_port_config->options).c_cflag)  &= ~(ICANON | ECHO | ECHOE | ISIG | ECHONL);
        *(&(serial_port_config->options).c_cflag)  &= ~(OPOST | ONLCR);
        *(&(serial_port_config->options).c_cflag)  &= ~(CSTOPB | PARENB);
        ...
        *(&(serial_port_config->options).c_cflag)  |= CREAD | CLOCAL | ASYNC_LOW_LATENCY;
